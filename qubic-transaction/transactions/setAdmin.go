...
func run() error {
	sourceID := "PXABYVDPJRRDKELEYSHZWJCBEFJCNERNKKUWXHANCDPQEFGDIUGUGAUBBCYK"      // my ID (current adminID) - public key account 1
	destinationID := "LAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAKPTJ" // contract ID
	adminID := "FPOBXDXMCSNZLCFWIERCYWYSQBLDPXRMHSDKVIFMTESIUGNFLAZLDLTDYMGF"       // New admin ID - public key account2
	...
	subSeed, err := types.GetSubSeed("hnwizobbscgmqfvosckyrrigrvtfkucaqagguxscmyewagynkrwychd") //public key account 1
	if err != nil {
