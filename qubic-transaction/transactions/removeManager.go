...
func run() error {
	sourceID := "PXABYVDPJRRDKELEYSHZWJCBEFJCNERNKKUWXHANCDPQEFGDIUGUGAUBBCYK"      // admin ID (current admin) - public key account 2
	destinationID := "LAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAKPTJ" // contract ID
	managerID := "MNGERABCDXPLKWYSQTRFHDLKJVRFMNDQLPKFDIFNSRTQLWOSJXQPLKTZMQGJ"     // New manager ID

	srcID := types.Identity(sourceID)
	destID := types.Identity(destinationID)
	mngID := types.Identity(managerID)
	...
	managerPubKey, err := mngID.ToPubKey(false)
	if err != nil {
		return errors.Wrap(err, "converting manager id string to pubkey")
	}
	...
	tx := types.Transaction{
		...
		InputType:            5,        // addManager input type 4
	...
	subSeed, err := types.GetSubSeed("mzzubckzujqlkddwakfvgddvxvrgparnwvsqglobuovgloxkjbmxewb") //private key account2
	if err != nil {
